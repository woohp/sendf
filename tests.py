...
class SendFTestCase(TestCase):
    ...
    def test_raises_error_when_upnp_device_not_found(self) -> None:
        with mock.patch('upnp.discover', return_value=None), \
                self.assertRaisesRegex(RuntimeError, 'IGD not found'):
    ...
    def test_single_file_with_custom_name(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.content, self._file_content)
        self.assertEqual(response.headers['content-disposition'], 'attachment; filename="customname.py"')
        self.assertEqual(response.headers['content-type'], 'text/x-python; charset=utf-8')
    ...
    def test_multiple_files(self):
        ...
        response = client.get(f'/{sendf.uuid}')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.headers['content-disposition'], 'attachment; filename="Archive.tar"')
    ...
    def test_multiple_files_with_custom_name(self):
        sendf = SendF(['tests.py', 'sendf.py'], output_fname='customname.tar')
        app = Starlette(debug=True, routes=[Route('/{uuid}', sendf.call)])
        client = TestClient(app)
        response = client.get(f'/{sendf.uuid}')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.headers['content-disposition'], 'attachment; filename="customname.tar"')
        self.assertEqual(response.headers['content-type'], 'application/x-tar')
